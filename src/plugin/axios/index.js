...
import { getRegion} from '@/utils/auth'
// 记录和显示错误
  ...
  config => {
    ...
    if (!(/^https:\/\/|http:\/\//.test(config.url))) {
      ...
      if (token && token !== 'undefined') {
        // 让每个请求携带token-- ['Authorization']为自定义key 请根据实际情况自行修改 'Bearer ' +
        config.headers['X-Token'] = token
      }
      if (getRegion()) {
        config.headers['X-CTYUN-REGION'] = getRegion() // 让每个请求携带自定义token 请根据实际情况自行修改
      }
    }
    return config
  },
